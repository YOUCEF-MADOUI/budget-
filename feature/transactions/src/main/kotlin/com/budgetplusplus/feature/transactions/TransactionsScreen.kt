...
fun TransactionsScreen(onBack: () -> Unit, viewModel: TransactionsViewModel = hiltViewModel()) {
    ...
    Scaffold(topBar = { BudgetTopAppBar(stringResource(R.string.transactions_title), onBackClick = onBack) }, floatingActionButton = { FloatingActionButton(onClick = { if (accounts.isNotEmpty()) add = true }) { Text(stringResource(R.string.transactions_add_symbol)) } }) { padding ->
        if (rows.isEmpty()) BudgetEmptyState(stringResource(R.string.transactions_empty), stringResource(if (accounts.isEmpty()) R.string.transactions_need_account else R.string.transactions_empty_message), Modifier.padding(padding))
            ...
            TransactionRow(category = transaction.categoryCustomName ?: transaction.categoryNameKey?.let { transactionCategory(it) } ?: typeLabel(transaction.type), description = transaction.description, date = DateFormat.getDateInstance(DateFormat.MEDIUM).format(Date(transaction.occurredAt)), account = accountLabel, amountMinor = transaction.amountMinor, currencyCode = transaction.currencyCode, type = visualType(transaction.type), categoryIcon = BudgetIcons.Category, onClick = { viewModel.delete(transaction.id) })
        } }
    }
