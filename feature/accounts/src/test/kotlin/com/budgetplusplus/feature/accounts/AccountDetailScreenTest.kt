...
class AccountDetailScreenTest {
 ...
 @Test fun `detail renders totals history and quick actions`(){compose.waitUntil{search.calls>0};compose.onNodeWithText("Cash").assertExists();compose.onNodeWithText("Revenus").assertExists();compose.onNodeWithText("Dépenses").assertExists();compose.onNodeWithText("Expense").assertExists();compose.onNodeWithText("Ajouter une dépense").assertExists();compose.runOnIdle{viewModel.toggle("tx");viewModel.reassign("b"){}};compose.waitUntil{accounts.moved!=null};assertEquals(setOf("tx"),accounts.moved?.third)}
}
private class DetailAccounts:AccountRepository{val list=flowOf(listOf(Account("a","Cash",AccountType.CASH,"DZD",1000,900),Account("b","Bank",AccountType.BANK,"DZD",0,0)));var moved:Triple<String,String,Set<String>>?=null;override fun observeAccounts()=list;override fun observeTotals(id:String)=flowOf(AccountOperationTotals(0,100,1));override suspend fun create(name:String,type:AccountType,initialBalanceMinor:Long,currencyCode:String)=Unit;override suspend fun update(id:String,name:String,type:AccountType,iconKey:String,colorKey:String,description:String,displayOrder:Int)=Unit;override suspend fun setArchived(id:String,archived:Boolean)=Unit;override suspend fun reassignOperations(sourceAccountId:String,targetAccountId:String,transactionIds:Set<String>){moved=Triple(sourceAccountId,targetAccountId,transactionIds)};override suspend fun deleteAndReassign(sourceAccountId:String,targetAccountId:String?)=Unit}
