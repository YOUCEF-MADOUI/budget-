...
@Composable fun SearchScreen(onBack:()->Unit,onEdit:(String)->Unit,vm:SearchViewModel=hiltViewModel()){val f by vm.filter.collectAsStateWithLifecycle();val s by vm.state.collectAsStateWithLifecycle();val accounts by vm.accounts.collectAsStateWithLifecycle();val categories by vm.categories.collectAsStateWithLifecycle();val subs by vm.subs.collectAsStateWithLifecycle();var filters by remember{mutableStateOf(false)}
 Scaffold(topBar={BudgetTopAppBar(stringResource(R.string.search_title),onBackClick=onBack)}){p->Column(Modifier.padding(p).fillMaxSize()){BudgetSearchBar(f.text,{vm.update(f.copy(text=it))},Modifier.padding(horizontal=16.dp),stringResource(R.string.search_hint));Row(Modifier.padding(horizontal=16.dp).fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){TextButton({filters=!filters}){Text(stringResource(R.string.search_filters))};TextButton(vm::reset){Text(stringResource(R.string.search_reset))};Button(vm::apply){Text(stringResource(R.string.search_apply))}}
  ...
  when{ s.error->BudgetErrorState(stringResource(R.string.search_error_title),stringResource(R.string.search_error_message),vm::loadMore,Modifier.fillMaxSize());s.items.isEmpty()&&s.loading->BudgetLoadingIndicator(Modifier.fillMaxSize());s.items.isEmpty()->BudgetEmptyState(stringResource(R.string.search_empty),stringResource(R.string.search_empty_message),Modifier.fillMaxSize());else->LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(12.dp)){items(s.items,key={it.id}){t->ResultRow(t,onEdit)};if(!s.end)item{LaunchedEffect(s.items.size){vm.loadMore()};Box(Modifier.fillMaxWidth().padding(16.dp)){CircularProgressIndicator()}}}}
 }}}
...
@Composable private fun ResultRow(t:FinanceTransaction,onEdit:(String)->Unit){val account=t.destinationAccountName?.let{stringResource(R.string.search_transfer_accounts,t.accountName,it)}?:t.accountName;TransactionRow(t.categoryCustomName?:t.subcategoryName?:typeName(t.type),t.description,DateFormat.getDateInstance(DateFormat.MEDIUM).format(Date(t.occurredAt)),account,t.amountMinor,t.currencyCode,when(t.type){TransactionType.INCOME->TransactionVisualType.Income;TransactionType.EXPENSE->TransactionVisualType.Expense;TransactionType.TRANSFER->TransactionVisualType.Transfer},BudgetIcons.Category,onClick={onEdit(t.id)})}
