...
class RepositoryIntegrationTest {
 private lateinit var db:BudgetPlusDatabase;private lateinit var accounts:AccountRepository;private lateinit var categories:CategoryRepository;private lateinit var transactions:TransactionRepository
 ...
 @Test fun `editing amount account and type atomically recalculates both balances`()=runBlocking{accounts.create("Cash",AccountType.CASH,10_000);accounts.create("Bank",AccountType.BANK,0);categories.create("Food",CategoryKind.EXPENSE);categories.create("Salary",CategoryKind.INCOME);val a=accounts.observeAccounts().first().associateBy{it.name};val cats=categories.observeCategories().first().associateBy{it.customName};transactions.create(TransactionType.EXPENSE,1_000,a.getValue("Cash").id,null,cats.getValue("Food").id,"Old");val id=transactions.observeTransactions().first().single().id;transactions.update(id,TransactionType.INCOME,2_500,a.getValue("Bank").id,null,cats.getValue("Salary").id,"Edited",null,LocalDate.now().toString());val balances=accounts.observeAccounts().first().associateBy{it.name};assertEquals(10_000,balances.getValue("Cash").currentBalanceMinor);assertEquals(2_500,balances.getValue("Bank").currentBalanceMinor);val edited=requireNotNull(transactions.get(id));assertEquals(id,edited.id);assertEquals(TransactionType.INCOME,edited.type);assertEquals("Edited",edited.description)}
