...
class LocalBackupRepository @Inject constructor(@ApplicationContext private val context:Context,private val database:BudgetPlusDatabase):BackupRepository{
 ...
 private fun csvSpec(value:CsvDataset)=when(value){
  CsvDataset.ACCOUNTS->CsvSpec(listOf("id","name","type","currency_code","initial_balance_minor","archived"),"SELECT id,name,type,currency_code,initial_balance_minor,is_archived FROM accounts WHERE deleted_at IS NULL ORDER BY name")
