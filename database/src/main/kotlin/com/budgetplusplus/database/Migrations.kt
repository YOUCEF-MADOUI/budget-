...
val MIGRATION_7_8 = object : Migration(7,8){override fun migrate(db:SupportSQLiteDatabase){db.execSQL("ALTER TABLE accounts ADD COLUMN description TEXT NOT NULL DEFAULT ''")}}

val MIGRATION_8_9 = object : Migration(8,9){override fun migrate(db:SupportSQLiteDatabase){db.execSQL("ALTER TABLE accounts ADD COLUMN parent_account_id TEXT REFERENCES accounts(id) ON UPDATE NO ACTION ON DELETE RESTRICT");db.execSQL("CREATE INDEX IF NOT EXISTS index_accounts_parent_account_id_display_order_deleted_at ON accounts(parent_account_id,display_order,deleted_at)");createAccountHierarchyInfrastructure(db)}}

val MIGRATION_9_10 = object : Migration(9,10){override fun migrate(db:SupportSQLiteDatabase){
 db.execSQL("""CREATE TABLE IF NOT EXISTS future_payments(id TEXT NOT NULL PRIMARY KEY,workspace_id TEXT NOT NULL,name TEXT NOT NULL,description TEXT NOT NULL,planned_amount_minor INTEGER NOT NULL,currency_code TEXT NOT NULL,due_date TEXT NOT NULL,category_id TEXT NOT NULL,subcategory_id TEXT,planned_account_id TEXT,priority TEXT NOT NULL,reminder_enabled INTEGER NOT NULL,reminder_days_before INTEGER NOT NULL,notes TEXT NOT NULL,cancelled INTEGER NOT NULL,created_at INTEGER NOT NULL,updated_at INTEGER NOT NULL,deleted_at INTEGER,FOREIGN KEY(workspace_id) REFERENCES workspaces(id) ON UPDATE NO ACTION ON DELETE CASCADE,FOREIGN KEY(category_id) REFERENCES categories(id) ON UPDATE NO ACTION ON DELETE RESTRICT,FOREIGN KEY(subcategory_id) REFERENCES subcategories(id) ON UPDATE NO ACTION ON DELETE SET NULL,FOREIGN KEY(planned_account_id) REFERENCES accounts(id) ON UPDATE NO ACTION ON DELETE SET NULL)""")
 db.execSQL("CREATE INDEX IF NOT EXISTS index_future_payments_workspace_id_due_date_cancelled_deleted_at ON future_payments(workspace_id,due_date,cancelled,deleted_at)");db.execSQL("CREATE INDEX IF NOT EXISTS index_future_payments_category_id ON future_payments(category_id)");db.execSQL("CREATE INDEX IF NOT EXISTS index_future_payments_subcategory_id ON future_payments(subcategory_id)");db.execSQL("CREATE INDEX IF NOT EXISTS index_future_payments_planned_account_id ON future_payments(planned_account_id)");db.execSQL("CREATE INDEX IF NOT EXISTS index_future_payments_priority_due_date ON future_payments(priority,due_date)")
 db.execSQL("""CREATE TABLE IF NOT EXISTS due_payments(id TEXT NOT NULL PRIMARY KEY,due_id TEXT NOT NULL,transaction_id TEXT NOT NULL,amount_minor INTEGER NOT NULL,paid_date TEXT NOT NULL,created_at INTEGER NOT NULL,cancelled_at INTEGER,FOREIGN KEY(due_id) REFERENCES future_payments(id) ON UPDATE NO ACTION ON DELETE CASCADE,FOREIGN KEY(transaction_id) REFERENCES finance_transactions(id) ON UPDATE NO ACTION ON DELETE RESTRICT)""");db.execSQL("CREATE INDEX IF NOT EXISTS index_due_payments_due_id_cancelled_at ON due_payments(due_id,cancelled_at)");db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_due_payments_transaction_id ON due_payments(transaction_id)")
 db.execSQL("""CREATE TABLE IF NOT EXISTS due_reminder_events(due_id TEXT NOT NULL,reminder_date TEXT NOT NULL,notified_at INTEGER NOT NULL,PRIMARY KEY(due_id,reminder_date),FOREIGN KEY(due_id) REFERENCES future_payments(id) ON UPDATE NO ACTION ON DELETE CASCADE)""");db.execSQL("CREATE INDEX IF NOT EXISTS index_due_reminder_events_due_id ON due_reminder_events(due_id)")
}}

val MIGRATION_10_11 = object : Migration(10,11){override fun migrate(db:SupportSQLiteDatabase){
 db.execSQL("CREATE TABLE IF NOT EXISTS media_assets(id TEXT NOT NULL PRIMARY KEY,file_name TEXT NOT NULL,thumbnail_file_name TEXT NOT NULL,width INTEGER NOT NULL,height INTEGER NOT NULL,size_bytes INTEGER NOT NULL,created_at INTEGER NOT NULL,deleted_at INTEGER)");db.execSQL("CREATE INDEX IF NOT EXISTS index_media_assets_deleted_at ON media_assets(deleted_at)")
 db.execSQL("""CREATE TABLE IF NOT EXISTS favorites(id TEXT NOT NULL PRIMARY KEY,workspace_id TEXT NOT NULL,name TEXT NOT NULL,media_id TEXT,icon_key TEXT NOT NULL,color_key TEXT NOT NULL,unit_price_minor INTEGER NOT NULL,currency_code TEXT NOT NULL,account_id TEXT NOT NULL,category_id TEXT NOT NULL,subcategory_id TEXT,description TEXT NOT NULL,behavior TEXT NOT NULL,display_order INTEGER NOT NULL,is_archived INTEGER NOT NULL,created_at INTEGER NOT NULL,updated_at INTEGER NOT NULL,deleted_at INTEGER,FOREIGN KEY(workspace_id) REFERENCES workspaces(id) ON UPDATE NO ACTION ON DELETE CASCADE,FOREIGN KEY(media_id) REFERENCES media_assets(id) ON UPDATE NO ACTION ON DELETE SET NULL,FOREIGN KEY(account_id) REFERENCES accounts(id) ON UPDATE NO ACTION ON DELETE RESTRICT,FOREIGN KEY(category_id) REFERENCES categories(id) ON UPDATE NO ACTION ON DELETE RESTRICT,FOREIGN KEY(subcategory_id) REFERENCES subcategories(id) ON UPDATE NO ACTION ON DELETE SET NULL)""");db.execSQL("CREATE INDEX IF NOT EXISTS index_favorites_workspace_id_is_archived_display_order ON favorites(workspace_id,is_archived,display_order)");listOf("media_id","account_id","category_id","subcategory_id").forEach{db.execSQL("CREATE INDEX IF NOT EXISTS index_favorites_${it} ON favorites(${it})")}
 db.execSQL("ALTER TABLE accounts ADD COLUMN media_id TEXT REFERENCES media_assets(id) ON UPDATE NO ACTION ON DELETE SET NULL");db.execSQL("CREATE INDEX IF NOT EXISTS index_accounts_media_id ON accounts(media_id)")
 db.execSQL("ALTER TABLE categories ADD COLUMN media_id TEXT REFERENCES media_assets(id) ON UPDATE NO ACTION ON DELETE SET NULL");db.execSQL("CREATE INDEX IF NOT EXISTS index_categories_media_id ON categories(media_id)")
 db.execSQL("ALTER TABLE finance_transactions ADD COLUMN media_id TEXT REFERENCES media_assets(id) ON UPDATE NO ACTION ON DELETE SET NULL");db.execSQL("ALTER TABLE finance_transactions ADD COLUMN favorite_id TEXT REFERENCES favorites(id) ON UPDATE NO ACTION ON DELETE SET NULL");db.execSQL("ALTER TABLE finance_transactions ADD COLUMN unit_price_minor INTEGER");db.execSQL("ALTER TABLE finance_transactions ADD COLUMN quantity INTEGER NOT NULL DEFAULT 1");db.execSQL("CREATE INDEX IF NOT EXISTS index_finance_transactions_media_id ON finance_transactions(media_id)");db.execSQL("CREATE INDEX IF NOT EXISTS index_finance_transactions_favorite_id ON finance_transactions(favorite_id)")
 db.execSQL("""CREATE TABLE IF NOT EXISTS favorite_click_batches(favorite_id TEXT NOT NULL PRIMARY KEY,transaction_id TEXT NOT NULL,last_clicked_at INTEGER NOT NULL,quantity INTEGER NOT NULL,FOREIGN KEY(favorite_id) REFERENCES favorites(id) ON UPDATE NO ACTION ON DELETE CASCADE,FOREIGN KEY(transaction_id) REFERENCES finance_transactions(id) ON UPDATE NO ACTION ON DELETE CASCADE)""");db.execSQL("CREATE INDEX IF NOT EXISTS index_favorite_click_batches_transaction_id ON favorite_click_batches(transaction_id)")
}}
