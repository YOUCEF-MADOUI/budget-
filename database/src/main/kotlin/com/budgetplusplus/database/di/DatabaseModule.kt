        ...
        Room.databaseBuilder(context, BudgetPlusDatabase::class.java, BudgetPlusDatabase.NAME)
            .addCallback(object : RoomDatabase.Callback() {
                override fun onCreate(db: SupportSQLiteDatabase) {
                    ...
                    db.execSQL("INSERT INTO workspaces VALUES (?, ?, ?, ?, 1, ?, ?, NULL)", arrayOf(BudgetPlusDatabase.DEFAULT_WORKSPACE_ID, "Budget++", "PERSONAL", "DZD", now, now))
                    val seeds = listOf(
                    ...
                    seeds.forEachIndexed { index, (id, kind, key) ->
                        db.execSQL("INSERT INTO categories (id, workspace_id, kind, name_key, custom_name, icon_key, color_key, is_system, is_archived, display_order, created_at, updated_at, deleted_at) VALUES (?, ?, ?, ?, NULL, 'category', 'primary', 1, 0, ?, ?, ?, NULL)", arrayOf(id, BudgetPlusDatabase.DEFAULT_WORKSPACE_ID, kind, key, index, now, now))
                    }
                }
            }).build()
