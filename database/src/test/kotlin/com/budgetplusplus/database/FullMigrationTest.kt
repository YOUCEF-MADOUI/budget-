...
class FullMigrationTest {
 ...
 @Test fun `migrates a populated schema from version one through six`(){
  ...
  val migrated=Room.databaseBuilder(context,BudgetPlusDatabase::class.java,name).allowMainThreadQueries().addMigrations(MIGRATION_1_2,MIGRATION_2_3,MIGRATION_3_4,MIGRATION_4_5,MIGRATION_5_6,MIGRATION_6_7,MIGRATION_7_8,MIGRATION_8_9).build();assertEquals(9,migrated.openHelper.writableDatabase.version);assertTrue(table(migrated.openHelper.writableDatabase,"subcategories"));assertTrue(table(migrated.openHelper.writableDatabase,"budgets"));assertTrue(table(migrated.openHelper.writableDatabase,"recurring_transactions"));assertTrue(table(migrated.openHelper.writableDatabase,"finance_transactions_fts"));migrated.close()
 }
