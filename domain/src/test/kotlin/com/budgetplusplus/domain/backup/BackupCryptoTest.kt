...
class BackupCryptoTest {
 ...
 @Test fun `round trips encrypted version two backup`(){val out=ByteArrayOutputStream();BackupCrypto.encrypt(database,6,"correct horse".toCharArray(),out);val decoded=BackupCrypto.decrypt(ByteArrayInputStream(out.toByteArray()),"correct horse".toCharArray());assertArrayEquals(database,decoded.databaseBytes);assertEquals(2,decoded.formatVersion);assertEquals(6,decoded.schemaVersion)}
